...
func TestReportsAPIProduction(t *testing.T) {
	// 1. 加载环境变量
	envPaths := []string{
		".env.prod",          // 当前目录
		"../.env.prod",       // 上级目录
	...
	if !envLoaded {
		t.Skip("跳过 Reports API 生产环境测试：未找到 .env 或 .env.prod1 文件")
	}
	...
	t.Run("获取报告列表", func(t *testing.T) {
		...
		request := &reports.GetReportsRequest{
			ReportTypes:    []string{"GET_FLAT_FILE_OPEN_LISTINGS_DATA"}, // 报告类型
	...
	t.Run("创建报告", func(t *testing.T) {
		...
		request := &reports.CreateReportRequest{
			ReportType:     "GET_FLAT_FILE_OPEN_LISTINGS_DATA", // 报告类型
			MarketplaceIds: []string{"ATVPDKIKX0DER"},          // 美国生产环境
		}
