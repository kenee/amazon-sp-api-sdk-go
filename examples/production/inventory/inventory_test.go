...
func TestInventoryAPIProduction(t *testing.T) {
	// 1. 加载环境变量
	envPaths := []string{
		".env.prod",          // 当前目录
		"../.env.prod",       // 上级目录
	...
	if !envLoaded {
		t.Skip("跳过库存 API 生产环境测试：未找到 .env 或 .env.prod1 文件")
	}
	...
	t.Run("获取库存摘要 (完整版本)", func(t *testing.T) {
		...
		request := &inventory.GetInventorySummariesRequest{
			GranularityType: "Marketplace",   // 粒度类型
			GranularityId:   "ATVPDKIKX0DER", // 粒度ID (Marketplace ID)
			SellerId:        sellerId,
			MarketplaceIds:  marketplaceIds, // 美国生产环境
			Details:         true,           // 是否包含详细信息
			// 其他参数保持null，与PHP示例一致
		}
