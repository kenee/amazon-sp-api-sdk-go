...
func loadTestConfig(t *testing.T) *client.Configuration {
	...
	if !envLoaded {
		t.Skip("跳过测试：未找到 .env 或 .env.prod1 文件")
		return nil
	}
...
func TestListingFlow_Setup(t *testing.T) {
	...
	if !envLoaded {
		t.Skip("跳过刊登流程生产环境测试：未找到 .env 或 .env.prod1 文件")
		return
	}
...
func TestCompleteListingFlowProduction(t *testing.T) {
	t.Log("🚀 开始完整的 Amazon 刊登发品流程测试")

	// 检查环境变量
	if os.Getenv("SP_API_CLIENT_ID") == "" {
		t.Skip("跳过真实 API 测试：缺少 SP-API 凭据")
		return
	}
